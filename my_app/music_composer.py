...
def main():
    st.sidebar.markdown("Music Composer")
    hide_default_format = """
           <style>
    ...
    with st.container():
        ...
        with col2:
            genre = st.selectbox(
                'Select the genre',
                ("-", 'Rock ', 'Classical', 'Folk', 'HipHop', 'Romance')
            )
            mood = st.selectbox(
                'Select the mood',
                ("-", 'upbeat', 'calm', 'Melancholic', 'Motivational')
            )
            Instruments = st.selectbox(
                'Select the Instrument',
                ("-", 'Violin ', 'Guitar', 'Piano', 'Flute', 'Drums')
            )
    ...
    if generate_button:
        ...
        prompt = f"Create a music based on their users genre {genre}, mood {mood}, instrument being played {Instruments}"
