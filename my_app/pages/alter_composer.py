...
def main():
    st.sidebar.markdown("Alter Music Com")
    hide_default_format = """
           <style>
    ...
    st.title("Alter Music Com🎵")

    with st.expander("See explanation"):
        st.write("Versatile AI music composer crafting original pieces in various genres.")

    with st.container():
        col1, col2 = st.columns([2, 1])

        # Add text area to the first column
        with col2:
            text_area = st.text_area("Enter your description.......", height=210)

        # Add genre selection to the second column
        with col1:
            famous_composer = st.selectbox(
                'Select the Music Composer',
                ("-", 'Emma Watson ', 'Kendrick Lamar', 'Justin Bieber', 'Beyoncé', 'A R Rahman')
            )
    ...
    with st.container():
        ...
        with col3:
            music_name = st.text_area("Give a name to your file")
        with col4:
    ...
    if generate_button:
        ...
        prompt = f"Create a music of a famous music composer {famous_composer} with their new genre {text_area}"
